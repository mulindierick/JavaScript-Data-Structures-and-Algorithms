// use this when you want to data or find its frequency 
// this pattern uses objects or sets to collect values/frequencies of values
// This can often avoid the need for nested loop or O(N**2) operations with arrays and strings
// its usually O(N)

// example 1

// Write a function called same, which accepts two arrays. The function should return true if every value in the array
// has a corresponding squared in the second array. The frequency of values must be the same.
...
function same(arr1, arr2) {
  ...
  for (const element of arr1) {
    fCounter1[element] = ++fCounter1[element] || 1;
  }
  // add elements to the second object
  for (const element of arr2) {
    fCounter2[element] = ++fCounter2[element] || 1;
  }
